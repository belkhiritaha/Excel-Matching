...
#from .compare import Tax_Values
...
if inputQuestion == "n":
    ...
    print("0: SH Codes, 1: Declared Value, 2: Tax codes, 3: Tax Values, 4: Quantities")
...
for i in range(len(dataset)):
    ...
    TaxC = dataset.iloc[i]['Column' + str(columns[2])]
    # check if value is NAN
    if (not pd.isna(TaxC)) and (TaxC != '!'):
        Tax_Codes.append((TaxC, articleCount))

    TaxV = dataset.iloc[i]['Column'+ str(columns[3])]
    # check if value is NAN
    if (not pd.isna(TaxV)) and (TaxV != '!'):
        Tax_Values.append((TaxV))
